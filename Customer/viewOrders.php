...
$sqlOrders = "SELECT * FROM orders WHERE customer_id = '$customer_id'";
$resultOrders = mysqli_query($con, $sqlOrders);

// Fetch color from customer_info for pricing
$sqlColor = "SELECT color FROM customer_info WHERE id = '$customer_id'";
    ...
    if ($resultOrders && mysqli_num_rows($resultOrders) > 0) {
        ...
        while ($orderRow = mysqli_fetch_assoc($resultOrders)) {
            ...
            if ($orderRow['order_state'] === 'Cancelled') {
                ...
                // Check if the bill is already generated for this order
                $order_id = $orderRow['order_id'];
                $sqlCheckBill = "SELECT * FROM bill WHERE order_id = '$order_id'";
                $resultCheckBill = mysqli_query($con, $sqlCheckBill);

                if ($resultCheckBill && mysqli_num_rows($resultCheckBill) === 0) {
                    // If the bill is not generated, provide an option to generate
                    echo "<td><a href='generateBill.php?order_id={$orderRow['order_id']}'>Generate Bill</a></td>";
                } else {
                    // Display a button to view the bill
                    $sqlGetBillId = "SELECT bill_id FROM bill WHERE order_id = '$order_id'";
